...
Status PartitionedHashJoinProbeLocalState::recovery_probe_blocks_from_disk(RuntimeState* state,
    ...
    has_data = false;
    if (!spilled_stream) {
        LOG(INFO) << "no probe data need to recovery for partition: " << partition_index
                  << ", node id: " << _parent->id() << ", task id: " << state->task_id();
        return Status::OK();
    }
...
Status PartitionedHashJoinProbeOperatorX::push(RuntimeState* state, vectorized::Block* input_block,
    ...
    if (eos) {
        LOG(INFO) << "probe push eos, node id: " << id() << ", task id: " << state->task_id();
    }
    if (rows == 0) {
        if (eos) {
...
Status PartitionedHashJoinProbeOperatorX::_setup_internal_operators(
    ...
    LOG(INFO) << "setup internal operators, node id: " << id() << ", task id: " << state->task_id()
              << ", partition index: " << local_state._partition_cursor;
...
Status PartitionedHashJoinProbeOperatorX::pull(doris::RuntimeState* state,
    ...
    if (local_state._need_to_setup_internal_operators) {
        LOG(INFO) << "need to setup internal operators, node id: " << id()
                  << ", task id: " << state->task_id()
                  << ", partition index: " << local_state._partition_cursor;
        *eos = false;
        bool has_data = false;
    ...
    while (_probe_operator->need_more_input_data(runtime_state)) {
        if (probe_blocks.empty()) {
            *eos = false;
            ...
            if (!has_data) {
                vectorized::Block block;
                LOG(INFO) << "push eos into internal probe operator: " << id()
                          << ", task id: " << state->task_id();

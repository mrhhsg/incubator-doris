...
Status PInternalServiceImpl::_multi_get(const PMultiGetRequest *request, PMultiGetResponse * response) {
    ...
    if (parallel) {
        ...
        for (size_t i = 0; i < parallel_n; ++i) {
            ...
            range.first = i * config::multi_get_per_batch;
            range.second = (i + 1) * config::multi_get_per_batch;
            if (i == parallel_n - 1) {
                // last one
                range.second = total_row_cnt;
            }
    ...
    RETURN_IF_ERROR(block.serialize(response->mutable_block(), &uncompressed_size, &compressed_size, segment_v2::CompressionTypePB::LZ4));
    return Status::OK();
}
